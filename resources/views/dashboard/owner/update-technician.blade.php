                ...
                <div class="card xl:col-span-2">
                        <div class="card-body flex flex-col p-6">
                            <header class="flex mb-5 items-center border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
                                <div class="flex-1">
                                    <div class="card-title text-slate-900 dark:text-white">Update Technician</div>
                                </div>
                            </header>
                            <div class="card-text h-full ">
                                <form class="space-y-4" method="post" action="{{ route('owner.updateTechnician', $technician->id) }}">
                                    @csrf
                                    @method('PUT')
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                                        <div class="input-area relative">
                                            <label for="full_name" class="form-label">Full Name</label>
                                            <input type="text" name="full_name" class="form-control @error('full_name') border-red-500 @enderror" placeholder="Full Name" value="{{ old('full_name',$technician->name) }}">
                                            @error('full_name')
                                        ...
                                        <div class="input-area relative">
                                            ...
                                            <input type="email" name="email" class="form-control @error('email') border-red-500 @enderror" placeholder="Enter Your Email" value="{{ old('email',$technician->email) }}">
                                            @error('email')
                                                <span class="text-red-500 text-sm">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div class="input-area relative">
                                            ...
                                            <input type="tel" name="phone_number" class="form-control @error('phone_number') border-red-500 @enderror" placeholder="Phone Number" value="{{ old('phone_number',$technician->phone_number) }}">
                                            @error('phone_number')
                                                <span class="text-red-500 text-sm">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    ...
                                    <button type="submit" class="btn inline-flex justify-center btn-dark">Update Technician</button>
                                </form>
                            </div>
                        </div>
                    </div>
